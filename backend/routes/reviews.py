...
def get_reviews(
    ...
    query = supabase.table("analysis_results").select("*").eq("source_type", "review")
...
def get_review_detail(record_id: str, authorization: str = Header(None)):
    """full detail of single review."""
    user_id, is_admin = get_current_user(authorization)
